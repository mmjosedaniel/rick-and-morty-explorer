...
Before each write-capable worker starts, the coordinator must use the [automatic write-lease guard](.codex/write-lease-guard.md) to capture the current Git worktree baseline and issue the exact lease ID, contract digest, allowed paths, and forbidden paths. After the worker stops, the coordinator must terminally close that same lease and may accept the handoff barrier only from a fresh `closed-compliant` result. A replayed close is recovery evidence only and also requires pinned `status` to report the original compliant receipt with no post-close drift. Any scope violation, forbidden or unleased endpoint change, scan-time instability, index or `HEAD` drift, ignore-control drift, state mismatch, tamper indication, or guard failure freezes further writes for coordinator triage. The guard cannot attribute a concurrent writer whose final net change is wholly inside an allowed endpoint. Workers must not invoke the guard, redefine a baseline, modify its ignored runtime state, or repair a violation by reverting user or peer work. A correction receives a new lease only after the prior lease is terminal and the coordinator has reconciled the working tree.
