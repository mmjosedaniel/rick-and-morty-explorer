...
For any bounded repository research, follow the objective `R0` through `R3` [research routing policy](.codex/README.md#research-work); scale agents by unresolved evidence and consequence rather than candidate count. When a task-scoped ExecPlan legitimately owns consequential decision work, keep its [Decision Review Contract](PLANS.md#decision-review-contract-decision-work-only) there. For non-task repository-policy research, keep the frozen capsule or comparison contract in the coordinating record and record a durable policy outcome in the execution log; do not invent a product task or ExecPlan solely to administer research. Preserve the primary thread as the sole authoritative decision-artifact writer and closure owner, and stop at owner-controlled approval boundaries. Research, analysis, drafting, and review agents remain read-only; a drafting agent may return one non-authoritative draft only after synthesis and any required pre-draft checkpoint.
