...
This repository is currently in its requirements and architecture phase. It contains the assessment contract, accepted architecture decisions including ADR-0011 for the planned TypeScript test harness and ADR-0012 for a build-first programmatic migration lifecycle, a target system module diagram, a graph-oriented implementation plan whose first implementation node is an operational walking skeleton, a root ExecPlan convention, completed TASK-001 and TASK-002 decision ExecPlans, an [active ready-to-execute TASK-016 character-image delivery decision ExecPlan](./docs/plans/TASK-016-character-image-delivery-decision.md), three project-scoped Codex agent definitions, derived Gherkin specifications, UI field-visibility and visual-foundation specifications, a proposed non-blocking Storybook pilot, execution records, and repository-specific workflow guidance. DG-001 and DG-002 are resolved, TASK-001 and TASK-002 are `Complete`, and TASK-003 and TASK-016 are ready by dependency but remain `Pending` and have not started; TASK-004 also remains `Pending`, and no migration runner, test harness, or character-image delivery behavior is implemented. The repository does not yet contain UI mockups, an application scaffold, Storybook configuration or stories, runnable web or API services, migrations, automated product tests, image assets, an ERD, or authoritative install, run, migration, import, and API-usage commands.
