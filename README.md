...
| [Project-scoped Codex agents](./.codex/README.md) | Reusable read-only and write-capable roles, model policy, decision and implementation topologies, review checkpoints, and permission boundaries | Product scope, task ownership, architectural approval, implementation status, or acceptance evidence |
| [Worker-first ExecPlan implementation workflow](./.codex/execplan-implementation-workflow.md) | Sequential write leases, Red-Green-Refactor handoffs, failure routing, independent review, and a copy-paste orchestration prompt | Task readiness, product scope, architecture approval, or passing implementation evidence |
...
| Implement behavior or fix a bug | [AGENTS.md](./AGENTS.md) -> [current status](#repository-status) -> exact requirement/AC IDs -> optional disposition -> relevant accepted ADRs -> exact [TASK](./docs/IMPLEMENTATION_PLAN.md#implementation-work-sequence) and gates -> only the mapped [SPEC/HS rules](./docs/specs/README.md#codex-rule-routing) -> applicable [UI design documents](./docs/ui/README.md) and recorded reversible decisions -> repository evidence | Follow the [worker-first ExecPlan implementation workflow](./.codex/execplan-implementation-workflow.md), including one Red-Green-Refactor cycle at a time, best-effort [flow metrics](./.codex/agent-flow-metrics.md), and task evidence/documentation updates. |
...
AGENTS.md
  ...
  -> repository evidence
  -> explicit sequential worker write leases when an ExecPlan is implemented
...
| Codex role, orchestration, hook, or flow-metrics policy | Owning `.codex` agent, workflow, hook, or metrics artifact | Root operating policy and documentation map, ExecPlan convention when execution contracts change, active plans only when their task-local semantics change, and the execution log; run documentation and configuration validation |
