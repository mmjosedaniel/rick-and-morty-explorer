...
This repository has completed TASK-003 and brought the owner-authorized TASK-004 implementation to closure readiness after establishing the initial requirements and architecture foundation. It contains the assessment contract; accepted architecture decisions including ADR-0011 for the TypeScript test harness, [ADR-0014](./docs/adrs/0014-persist-and-deliver-character-image-urls-directly.md) for persisting each exact validated official avatar URL and loading it directly, and [ADR-0015](./docs/adrs/0015-use-a-build-first-migration-lifecycle-with-exact-catalog-byte-lock-identity.md) as the current build-first migration lifecycle with a restricted lower-case ASCII, exact catalog-bound v2 identity for one fixed local/CI PostgreSQL profile; a target system module diagram; a graph-oriented implementation plan; completed task-scoped ExecPlans including [TASK-018](./docs/plans/completed/TASK-018-postgresql-migration-lock-identity-decision.md); an active [TASK-004 relational-persistence ExecPlan](./docs/plans/TASK-004-relational-persistence-from-migrations.md); five project-scoped Codex agent definitions; derived Gherkin specifications; UI specifications; execution records; and repository-specific workflow guidance. Fresh independent review returned `PASS` with no Blocker, Major, or Minor across all twenty hard gates and LOCK-INV-01 through LOCK-INV-21 on exact Proposed ADR-0015 SHA-256 `8B7B9EC9508DF01E57EA067344896814CD0B0B1B3D8083B889C7ED44AA5432B1`; the project owner explicitly approved those bytes on 2026-08-14. ADR-0015 is `Accepted`, ADR-0012 is preserved as `Superseded` history, DG-005 is `Resolved`, and TASK-018 is `Complete`. The project owner separately authorized TASK-004 execution on 2026-08-14. TASK-004 remains `In progress` with exactly TASK-002 and TASK-003 as dependencies while its closure joins are completed. Its relational migration, Sequelize adapter, lifecycle commands, failure/cleanup/lock behavior, prior committed clean-checkout packet, and prior hosted GitHub Actions run `31928215615` on exact commit `7b6d020aa9a26274bb1140c6098e803f6045618f` pass. The fresh integrated [TASK-004 acceptance review](./docs/reviews/2026-08-16-task-004-acceptance-review.md) returned `REVISE` because the artifact omitted two inherited TypeScript configuration inputs. One bounded TDD correction now authenticates all three compiler configurations, and a second bounded cycle makes emitted bytes independent of LF/CRLF checkout materialization while preserving exact output hashing. The current 20-input/24-file artifact passes unit 55/55, PostgreSQL 18.6 integration 67/67, application 2/2, and root build locally. A fresh committed checkout at `646e8cd5e4c3b5bc6f2b6e4446f8cddd6be21d46` passes all 35 controller checks and reproduces the exact same build and manifest identity; hosted CI, fresh re-review, and documentation closure remain. ADR-0014 remains `Accepted`; ADR-0001, ADR-0004, and ADR-0013 also remain preserved `Superseded` history. DG-001, DG-002, DG-004, DG-005, and DG-006 are resolved; TASK-001, TASK-002, TASK-003, TASK-016, TASK-017, and TASK-018 are `Complete`; and [AUTH-001](./docs/IMPLEMENTATION_PLAN.md#auth-001---character-image-content-rights-authorization) is `Authorized` only for this personal, educational, non-commercial portfolio under ADR-0014's exact direct-URL boundary and ordinary browser/intermediary caching.
...
The current evidence-based [TASK-004 acceptance review](./docs/reviews/2026-08-16-task-004-acceptance-review.md) records:
...
[DG-003](./docs/IMPLEMENTATION_PLAN.md#dg-003---frontend-graphql-client-and-query-cache) is the only pending architectural decision gate and applies to later frontend data-access work. TASK-004 remains `In progress`: the fresh independent `REVISE` findings and the subsequently exposed LF/CRLF emission instability are corrected, and the corrected committed clean checkout passes. Hosted CI on the corrected HEAD, fresh re-review, and documentation closure still remain. The task-scoped implementation evidence does not convert any incomplete product acceptance criterion into `Pass`.
