...
The current evidence-based [TASK-005 acceptance review](./docs/reviews/2026-08-18-task-005-acceptance-review.md) records:

| View | Current result |
|---|---|
| Minimum assessment | Fail: 4 of 12 acceptance criteria pass. TASK-005 completes AC-009 in addition to the already passing AC-007, AC-008, and AC-011; mutations, Redis, product UI, responsive behavior, final ERD, and complete delivery remain incomplete. |
| Repository baseline | Fail: TASK-005's deterministic import contribution and TASK-006's adopted OR-001, OR-007, and OR-008 contribution pass, but the minimum assessment and remaining adopted product commitments are incomplete. |

[DG-003](./docs/IMPLEMENTATION_PLAN.md#dg-003---frontend-graphql-client-and-query-cache) is `Resolved` by [Accepted ADR-0017](./docs/adrs/0017-use-tanstack-query-with-a-project-owned-typed-graphql-executor.md) after fresh final independent `PASS` and explicit project-owner approval of exact proposal SHA-256 `2A691BB6C2A025F264B9ABE70E93F45801B932404C4005090D0EB71808267158`. TASK-009 is `Complete`; TASK-010 is dependency-ready but remains pending separate execution authorization, and no frontend data-access implementation exists. TASK-004, TASK-005, and TASK-006 are `Complete`. TASK-005's three accepted behavior milestones, full root closure validation, zero-residue checks, fresh independent `PASS`, and documentation gate are recorded in its [completed ExecPlan](./docs/plans/completed/TASK-005-import-deterministic-15-character-baseline.md); TASK-006's four behavior milestones and closure remain recorded in its [completed ExecPlan](./docs/plans/completed/TASK-006-graphql-reads-filters-and-request-logging.md). Completed TASK-004 migration evidence plus TASK-005 initialization evidence makes SPEC-010 and AC-009 pass; HS-012 passes, and TASK-005's import contribution to SPEC-011 passes while full SPEC-011 still awaits TASK-008 mutations. The owner-approved allocation keeps bounded detail/comment reads in TASK-006 and mutations in TASK-008. This evidence changes AC-009 to `Pass`, yielding exactly AC-007, AC-008, AC-009, and AC-011 at 4/12; full NFR-003 still depends on Redis and later backend work.
...
The root command builds the API workspace and then runs its emitted zero-argument CLI. It requests public Rick and Morty character IDs 1 through 15 with finite timeouts and bounded retries, validates the complete batch and exact avatar association, and publishes the source-owned fields in one transaction. Repeating the command refreshes those fields without resetting favorites or comments. A fetch, validation, persistence, configuration, or close failure returns a non-zero result with a stable safe diagnostic; a post-commit invalidation failure emits a warning without rolling back PostgreSQL. TASK-007 owns the Redis implementation, so the direct command currently invokes the same post-commit seam through an intentional deferred no-op adapter and does not contact Redis.
