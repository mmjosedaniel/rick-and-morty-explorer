...
This repository has completed the owner-authorized TASK-003 through TASK-006 implementation tasks after establishing the initial requirements and architecture foundation. TASK-007 is `In progress` for an owner-authorized hosted-CI reproducibility correction. It contains the assessment contract; accepted architecture decisions including ADR-0011 for the TypeScript test harness, [ADR-0014](./docs/adrs/0014-persist-and-deliver-character-image-urls-directly.md) for persisting each exact validated official avatar URL and loading it directly, [ADR-0015](./docs/adrs/0015-use-a-build-first-migration-lifecycle-with-exact-catalog-byte-lock-identity.md) as the current build-first migration lifecycle with a restricted lower-case ASCII, exact catalog-bound v2 identity for one fixed local/CI PostgreSQL profile, and [ADR-0016](./docs/adrs/0016-use-milestone-slice-tdd-with-independent-test-and-implementation-ownership.md) for milestone-slice TDD with separate test and implementation ownership; a target system module diagram; a graph-oriented implementation plan; completed TASK-004 through TASK-006 and TASK-018 ExecPlans, plus the active [TASK-007 ExecPlan](./docs/plans/TASK-007-bounded-redis-cache-aside.md); ten project-scoped Codex agent definitions; derived Gherkin specifications; UI specifications; execution records; and repository-specific workflow guidance. ADR-0015 and ADR-0016 are `Accepted`, ADR-0010 and ADR-0012 are preserved as `Superseded` history, DG-005 is `Resolved`, and TASK-018 is `Complete`. The corrected [TASK-004 acceptance re-review](./docs/reviews/2026-08-16-task-004-acceptance-re-review.md) remains `PASS`. ADR-0014 remains `Accepted`; ADR-0001, ADR-0004, and ADR-0013 remain preserved `Superseded` history. DG-001, DG-002, DG-004, DG-005, and DG-006 are resolved; TASK-001 through TASK-006, TASK-016, TASK-017, and TASK-018 are `Complete`. TASK-007's historical acceptance, unit reproducibility, and acceptance re-review records remain point-in-time evidence, while the current [CI reproducibility review](./docs/reviews/2026-08-20-task-007-ci-reproducibility-review.md) returns `REVISE` because hosted run `32391250007` omitted the explicit Redis test namespace and failed integration 72/74. [AUTH-001](./docs/IMPLEMENTATION_PLAN.md#auth-001---character-image-content-rights-authorization) remains `Authorized` only for this personal, educational, non-commercial portfolio under ADR-0014's exact direct-URL boundary and ordinary browser/intermediary caching.

TASK-003 through TASK-006 are `Complete`; TASK-007 is `In progress` for Milestone 6. Its canonical/versioned finite-lived Redis search caching, bounded fail-open operations, Redis list-cache-demand-lazy connection, and scoped post-import SCAN/UNLINK invalidation remain implemented. PostgreSQL initializes on the first list, detail, or comment read; Redis remains unconnected until a list cache operation. Hosted pull-request run `32391250007` passed unit 139/139 but failed the two strict Redis integration scenarios because `.github/workflows/ci.yml` did not export `REDIS_NAMESPACE`. The active [ExecPlan](./docs/plans/TASK-007-bounded-redis-cache-aside.md) now owns a locally accepted run-isolated CI environment correction: focused workflow tests pass 2/2, real-Redis focus passes 3/3, root typecheck and exact cleanup pass, and fresh S1 review returns `PASS`. Hosted closure remains pending an authorized commit/push, a passing exact hosted run, and fresh integrated re-review. Favorite/comment mutations remain owned by TASK-008. AC-007, AC-008, AC-009, AC-010, and AC-011 product behavior passes; minimum-assessment readiness remains `Fail` at 5/12.
...
The current evidence-based [TASK-007 CI reproducibility review](./docs/reviews/2026-08-20-task-007-ci-reproducibility-review.md) records:
...
| Repository baseline | Fail: TASK-005's deterministic import, TASK-006's adopted OR-001/OR-007/OR-008, and TASK-007's Redis product behavior pass, but TASK-007 hosted closure and the remaining adopted product commitments are incomplete. |

[DG-003](./docs/IMPLEMENTATION_PLAN.md#dg-003---frontend-graphql-client-and-query-cache) is `Resolved` by [Accepted ADR-0017](./docs/adrs/0017-use-tanstack-query-with-a-project-owned-typed-graphql-executor.md) after fresh final independent `PASS` and explicit project-owner approval of exact proposal SHA-256 `2A691BB6C2A025F264B9ABE70E93F45801B932404C4005090D0EB71808267158`. TASK-009 is `Complete`; TASK-010 remains pending separate execution authorization. TASK-004 through TASK-006 are `Complete`; TASK-007 is `In progress` for Milestone 6. Completed migration/import evidence makes SPEC-010, HS-012, and AC-009 pass. TASK-007 product evidence makes SPEC-012, HS-013, AC-010, and the Redis portion of NFR-003 pass; its local CI namespace correction is accepted, but hosted closure still requires a passing run on the exact pushed candidate. Exactly AC-007, AC-008, AC-009, AC-010, and AC-011 product behavior passes at 5/12.
