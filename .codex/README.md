...
| Visible frontend implementation milestone | [Frontend quality skill](../.agents/skills/frontend-quality/SKILL.md) | Conditional `frontend-visual` profile in the implementation workflow; standard milestones remain unchanged |
...
| [`frontend_code_worker`](./agents/frontend-code-worker.toml) | Workspace write under an explicit lease | Perform Green with optional same-turn behavior-preserving Refactor only for an accepted `frontend-visual` milestone, reuse audit, and visual contract | Standard-profile work, test changes, independent design authority, evidence-document edits, architecture or scope decisions, status changes, or task closure |
...
Write-authorized implementation ExecPlans use the complementary [worker-first ExecPlan implementation workflow](./execplan-implementation-workflow.md). The primary retains integration, evidence acceptance, exception handling, approvals, authoritative status, and closure while delegating bounded edits through sequential path leases to `test_worker` and exactly one selected implementation context. The default `standard` profile uses the unchanged `code_worker`; only a milestone that materially changes rendered UI uses the conditional `frontend-visual` profile and `frontend_code_worker`.

For each coherent milestone slice, the primary creates one compact `Milestone Assignment Packet v2`. The standard route keeps the pre-existing packet shape; only a rendered-UI milestone adds the conditional `frontend-visual` marker and capsule before preflight. The primary keeps one test-worker instance and only the applicable implementation-worker instance alive for that milestone, then retires them after the milestone barrier. Agent persistence reduces repeated context loading but never persists write authority: every write turn has a fresh packet, baseline, digest, lease ID, and terminal close. If a live instance is unavailable, the primary respawns it from the same capsule.

The [frontend-quality skill](../.agents/skills/frontend-quality/SKILL.md) owns profile classification, the preflight reuse audit, and the visual-evidence boundary. A path under `apps/web` does not select the visual profile by itself: setup, generated operations, data access, cache behavior, and nonvisual routing remain standard. A frontend-visual milestone adds no new test owner, reviewer, risk tier, correction loop, or closure gate; it substitutes the specialized Green worker and gives the existing risk-routed reviewer the accepted reuse and browser evidence.
