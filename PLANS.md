...
Every production behavior follows one coherent milestone-slice Red-Green-Refactor cycle at a time under ADR-0016. The slice may include related scenarios that jointly prove one indivisible outcome, but it must not combine unrelated behavior. Under the worker-first workflow, `test_worker` performs preflight and either returns existing evidence, adds passing characterization coverage, or stops after proving the coherent Red contract. The primary coordinator accepts and freezes that boundary, and the applicable `code_worker` or `frontend_code_worker` performs the minimum Green and behavior-preserving Refactor sequentially. Record the classification, exact focused command and intended failure when Red applies, the passing Green command, and affected milestone validation in the ExecPlan and task evidence. Only a frontend-visual milestone additionally records its profile marker, accepted reuse dispositions, and assigned real-browser evidence without changing the ordinary risk route or claiming later responsive scope.
