...
For write-authorized implementation, use the [worker-first ExecPlan implementation workflow](.codex/execplan-implementation-workflow.md). The primary coordinator delegates bounded edits to separate `test_worker` and `code_worker` contexts sequentially, gives each assignment the compact `Milestone Assignment Packet v2`, and opens and terminally closes its exact path lease through the [automatic write-lease guard](.codex/write-lease-guard.md). One instance of each writer may remain live for bounded follow-ups within the same milestone, but every write turn requires a fresh packet and lease. The coordinator retains integration, evidence acceptance, exception handling, authoritative status, and closure. Decision work continues to use the sole-writer topology in [.codex/README.md](.codex/README.md).
...
When the worker-first workflow applies, define each milestone as one coherent observable contract. Record its preflight target, test and implementation ownership, risk tier, authority anchors, owned and frozen paths, focused and milestone commands, reusable evidence IDs, execution budget, expected handoff, and condition for advancing. Identify disjoint read-only or write-isolated branches that may run in parallel. The runtime coordinator instantiates `Milestone Assignment Packet v2` for each assignment and may expand context only to resolve a named uncertainty without expanding task or write scope. Keep Red, Green, and Refactor sequential within a slice. Describe exceptions as stop-and-triage conditions; the ExecPlan does not need a state-machine branch for every possible correction or failure. Metrics are optional and never a plan gate.
