...
|---|---|---|
| Custom agent schema and model route | Pass, client discovery pending | All 11 project agent TOMLs contain `name`, `description`, and `developer_instructions`; names are unique and match the repository filename convention. An isolated catalog-loader simulation selected `frontend_code_worker` with `gpt-5.6-sol`, `high`, `workspace-write`, and the expected fail-closed packet instructions. The unchanged `code_worker` remains `medium`; only a fresh trusted Codex task can prove client discovery. |
...
| Hook schema and behavior | Pass, trust pending | `python -m json.tool .codex/hooks.json` passed. A fresh temporary-repository simulation confirmed that both exact matchers accept `frontend_code_worker` and reject a near miss. The configured Windows command returned JSON `{}` for both Start and Stop, wrote two valid identity-preserving lifecycle events, and passed event validation with zero errors. `python -B .codex/metrics/agent_flow_metrics.py self-test` passed all eight behavioral groups. Persisted project-hook trust still requires the operator follow-up. |
...
A final short simulation created an isolated temporary Git repository, copied only the current hook, metrics script, and specialized-agent definition, and exercised the configured Windows hook command with `frontend_code_worker` Start and Stop payloads. The loader simulation found one role among 11 unique agent names with the expected Sol-high, sandbox, skill, capsule, reuse-audit, and packet constraints. Both matchers accepted the exact role and rejected `frontend_code_worker_extra`; event validation reported two valid events, zero errors, and zero replays. The exact temporary directory was verified under the operating-system temporary root and removed after the run. This is configuration and command evidence, not Codex client discovery or hook-trust evidence.
