...
Each scenario remains `Specified, not executed` until its owning task implements an ordinary automated check through the ADR-0011 boundary, every other applicable gate is resolved, and the authoritative validation command passes. The feature files themselves remain non-executable. TASK-004's accepted PostgreSQL lifecycle evidence completes only its migration-owned portion of SPEC-010; TASK-005 still owns the deterministic 15-character import, so full SPEC-010, AC-009, DEL-002, and AC-012 do not pass. Acceptance of ADR-0014 and Authorized AUTH-001 do not make an image scenario executed or passing. DG-002 and DG-005 do not block unrelated API or frontend work, DG-003 does not block backend or static-layout work, and resolved DG-006 plus AUTH-001 did not start TASK-003 or any image-delivery task.
...
| Express/GraphQL queries, filters, mutations, and all-Express request logs | SPEC-008, SPEC-009, SPEC-013, HS-008, HS-010, HS-014 | TASK-006 for Express-hosted queries, filters, and all-Express request logging; TASK-008 for mutation behavior |
