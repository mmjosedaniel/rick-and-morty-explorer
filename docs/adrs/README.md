...
Consequential choices that have not yet reached ADR evaluation are tracked as pending gates in the [implementation plan](../IMPLEMENTATION_PLAN.md#active-decision-gates). They are not accepted decisions and do not reserve ADR numbers. Non-architectural owner authorization is tracked separately through stable `AUTH-*` records. ADR-0011 is accepted and resolves DG-001; acceptance defines implementation direction but does not prove that the harness exists. ADR-0012 is accepted and historically resolves DG-002 with a build-first programmatic Umzug lifecycle. Its contract defines cross-platform canonical bytes and a fixed build-ID vector, binds the canonical migration mapping into the content ID, includes the common criteria matrix, and integrates the final POSIX path-alias follow-up. TASK-016 review later exposed that ADR-0012's NFC-based PostgreSQL migration-lock identity aliases valid distinct namespace names. ADR-0012 remains unchanged and Accepted, while pending DG-005 requires a separately researched, project-owner-approved reciprocal successor with a new migration-lock literal before TASK-004 may start. No migration-lock successor ADR number is reserved. Neither acceptance nor the 91/100 score proves that a migration runner or migrated database exists. ADR-0013 historically resolved DG-004 and superseded ADR-0004; ADR-0014 now supersedes ADR-0001 and ADR-0013, carries forward their unaffected constraints, and resolves DG-006. Acceptance did not itself select the separate [content-rights authorization disposition](./0014-persist-and-deliver-character-image-urls-directly.md#content-rights-authorization-boundary) or prove image-delivery behavior. [AUTH-001](../IMPLEMENTATION_PLAN.md#auth-001---character-image-content-rights-authorization) is `Authorized`; the implementation plan owns its continuity policy, while ADR-0014 independently owns the current direct-URL technical boundary.

## Portfolio evaluation

The current accepted decisions form one consistent baseline. ADR-0001, ADR-0004, and ADR-0013 are retained as Superseded history; ADR-0014 is the current successor for their carried-forward workspace and image-delivery constraints:
...
| Relational persistence and migrations | ADR-0003, ADR-0008, ADR-0012, ADR-0014; migration-lock identity remains pending under DG-005 |
