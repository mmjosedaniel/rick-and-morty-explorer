...
| [ADR-0011](./superseded/0011-define-the-typescript-test-harness.md) | Define the TypeScript test harness | Superseded | 92 | Accept | NFR-004, OR-001, OR-004, OR-007 |
...
| [ADR-0016](./0016-use-milestone-slice-tdd-with-independent-test-and-implementation-ownership.md) | Use milestone-slice TDD with independent test and implementation ownership | Accepted | 91 | Accept | NFR-004, OR-004, OR-007 |
| [ADR-0017](./0017-use-tanstack-query-with-a-project-owned-typed-graphql-executor.md) | Use TanStack Query with a project-owned typed GraphQL executor | Accepted | 93 | Accept | FR-FE-001, FR-FE-002, FR-FE-003, FR-FE-004, FR-FE-005, NFR-001, OR-003, AC-001, AC-002, AC-003, AC-004, AC-005 |
| [ADR-0018](./0018-define-the-typescript-test-harness-with-repository-owned-smoke-orchestration.md) | Define the TypeScript test harness with repository-owned smoke orchestration | Accepted | 92 | Accept | NFR-004, OR-001, OR-004, OR-007 |

Consequential choices that have not yet reached ADR evaluation are tracked as pending gates in the [implementation plan](../IMPLEMENTATION_PLAN.md#active-decision-gates). They are not accepted decisions and do not reserve ADR numbers. [ADR-0017](./0017-use-tanstack-query-with-a-project-owned-typed-graphql-executor.md) is Accepted after fresh final independent `PASS` and explicit project-owner approval of exact proposal SHA-256 `2A691BB6C2A025F264B9ABE70E93F45801B932404C4005090D0EB71808267158` on 2026-08-18; it resolves DG-003 and selects TanStack Query with a project-owned typed GraphQL executor without claiming frontend implementation. Non-architectural owner authorization is tracked separately through stable `AUTH-*` records. ADR-0018 is the current accepted DG-001 harness authority after ADR-0011's Windows cleanup reversal trigger occurred and the project owner approved the bounded repository-owned orchestrator; ADR-0011 is whole-record Superseded history. Acceptance defines direction and does not prove the outstanding clean/hosted lifecycle gate. [ADR-0016](./0016-use-milestone-slice-tdd-with-independent-test-and-implementation-ownership.md) is the current test-first workflow authority and supersedes ADR-0010 as a whole record; completed work retains ADR-0010 as its point-in-time authority, while pending and future work uses ADR-0016. Now-Superseded ADR-0012 historically resolved DG-002 with a build-first programmatic Umzug lifecycle. TASK-016 review later exposed that its NFC-based PostgreSQL migration-lock identity aliases valid distinct namespace names. [ADR-0015](./0015-use-a-build-first-migration-lifecycle-with-exact-catalog-byte-lock-identity.md) is the accepted whole-record successor prepared by completed [TASK-018](../plans/completed/TASK-018-postgresql-migration-lock-identity-decision.md). Fresh independent review returned `PASS` with no Blocker, Major, or Minor across all twenty hard gates and LOCK-INV-01 through LOCK-INV-21 on exact proposal SHA-256 `8B7B9EC9508DF01E57EA067344896814CD0B0B1B3D8083B889C7ED44AA5432B1`; the project owner explicitly approved those bytes on 2026-08-14. ADR-0015 is current, ADR-0012 is reciprocally Superseded, DG-005 is Resolved, and TASK-018 is Complete. Separate execution authorization later moved TASK-004 to `In progress`. The historical 2026-08-16 integrated review returned `REVISE` for two omitted inherited compiler configurations. One bounded correction authenticated all three TypeScript configurations, and a second correction made exact emitted bytes independent of LF/CRLF source materialization without normalizing published-output checksums. Local unit 55/55, build, PostgreSQL 18.6 integration 67/67, committed 35-check controller run `af1a90f61c5b8f9d`, and corrected hosted run `31947144452` pass; the LF and CRLF trees produce the same exact artifact identity. Fresh independent re-review returns `PASS` with no finding, the documentation gate passes, and TASK-004 is `Complete`. ADR-0013 historically resolved DG-004 and superseded ADR-0004; ADR-0014 now supersedes ADR-0001 and ADR-0013, carries forward their unaffected constraints, and resolves DG-006. Acceptance did not itself select the separate [content-rights authorization disposition](./0014-persist-and-deliver-character-image-urls-directly.md#content-rights-authorization-boundary) or prove image-delivery behavior. [AUTH-001](../IMPLEMENTATION_PLAN.md#auth-001---character-image-content-rights-authorization) is `Authorized` only within the recorded non-commercial portfolio/direct-URL scope; the implementation plan owns its scope-bound continuity and reopen policy, while ADR-0014 independently owns the current direct-URL technical boundary.

A post-closure [TASK-014 independent revision review](../reviews/2026-08-22-task-014-independent-revision-review.md) found that the executable server had not selected ADR-0006's accepted development-only explorer option. Active corrective [workflow `TASK-014-20260822-02`](../plans/TASK-014-delivery-correction.md) implements that existing decision without changing any ADR's status or meaning; ADR-0006 compatibility remains open until the corrected candidate passes review.

## Portfolio evaluation

The current accepted decisions form one consistent baseline. ADR-0001, ADR-0004, ADR-0010, ADR-0011, ADR-0012, and ADR-0013 are retained as Superseded history. ADR-0014 is the current successor for the carried-forward workspace and image-delivery constraints from ADR-0001 and ADR-0013, ADR-0015 is the current successor for ADR-0012's migration lifecycle, ADR-0016 is the current successor for ADR-0010's test-first workflow, ADR-0017 resolves DG-003 with the frontend data-access direction, and ADR-0018 carries forward ADR-0011's harness while replacing only smoke process ownership:
...
8. ADR-0016 defines milestone-slice TDD, preserves independent test and implementation ownership, prevents artificial Reds for existing behavior, applies milestone-level holistic review, and carries forward ADR-0010's test-relevance audit.
9. ADR-0018 maps that testing strategy to Vitest projects, jsdom, milestone-aware scope activation, external PostgreSQL and Redis isolation, and one Chromium-only Playwright smoke invoked by the repository-owned orchestrator; ADR-0011 preserves the prior Playwright-server-ownership boundary as Superseded history.
...
11. ADR-0017 assigns cached frontend server data to TanStack Query and request/media/error classification to a small project-owned typed GraphQL executor while preserving URL and component-local state ownership and exact post-mutation detail convergence.

Before any future work materially extends ADR-0015's migration lifecycle, ADR-0016 requires a proportional extension-cost watch covering mapped obligations, reused and new tests, projected milestone slices and handoffs, focused and full-suite runtime, and net-new custom lifecycle surface. Disproportionate observed cost triggers project-owner review and, if the accepted boundary must change, a successor ADR; ADR-0015 itself remains unchanged.
...
|---|---|
| Character list, sorting, detail, favorites, and comments | ADR-0005, ADR-0006, ADR-0009, ADR-0016; [ADR-0017](./0017-use-tanstack-query-with-a-project-owned-typed-graphql-executor.md) (`Accepted` client/cache direction) |
| GraphQL search and filtering | ADR-0003, ADR-0006, ADR-0007, ADR-0014, ADR-0016 |
| Relational persistence and migrations | ADR-0003, ADR-0008, ADR-0014, accepted ADR-0015; ADR-0012 is Superseded history |
| Initial set of 15 characters | ADR-0008, ADR-0016 |
...
| Redis search caching | ADR-0007, ADR-0016 |
| Responsive frontend and prescribed frontend stack | ADR-0002, ADR-0009, ADR-0014; [ADR-0017](./0017-use-tanstack-query-with-a-project-owned-typed-graphql-executor.md) (`Accepted` data-access direction) |
| Code quality | ADR-0002, ADR-0006, ADR-0007, ADR-0014, ADR-0016 |
| Automated test harness and execution boundaries | ADR-0018, ADR-0016; ADR-0010 and ADR-0011 are Superseded history |
| Request logging | ADR-0006, ADR-0016 |
...
| OR-004 - Frontend tests | Adopted | ADR-0016 | At least three frontend components or layouts require meaningful automated tests. |
...
| OR-007 - Backend tests | Adopted | ADR-0016 | The character-search query or service requires unit-level coverage. |
