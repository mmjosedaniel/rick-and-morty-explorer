...
| TASK-004 | In progress | TASK-002, TASK-003 | Implementation, owned Windows verification, and committed clean checkout pass; hosted CI, fresh independent review, and documentation closure remain |
...
- **Current evidence:** The repository builds and authenticates one 18-input/24-file native-ESM artifact, migrates an isolated PostgreSQL `180006|18.6|UTF8` namespace to the exact `characters`/`comments` model, exposes schema-aligned Sequelize mappings, and implements read-only status, idempotent up, bounded down/reapply, artifact and history authentication, atomic failure handling, redacted diagnostics, exact v2 lock behavior, same-namespace serialization, and disjoint-database/schema overlap. API and root commands preserve closed selector and result contracts. The latest task-level gates pass unit 53/53, application 2/2, build, integration 67/67, the owned 35-check Windows controller, and committed clean-checkout verification of exact commit `26d7dc125520d25d5e5c0c5ca9ddbc032f96e3d4`, all with exact infrastructure teardown.
- **Completion evidence:** Implementation, real Windows runtime, and committed clean-checkout joins pass. TASK-004 remains `In progress` until hosted-CI verification passes, a fresh independent review passes the exact integrated candidate, and the task-closure documentation gate is reconciled.
