...
- [ ] Create the preserved correction commit and repeat the complete authorized clean-checkout proof before accepting Milestone 1 or starting Milestone 2.
...
Milestone 1 has a locally Green and independently reviewed product candidate, but committed clean proof remains incomplete. The coordinator accepted `PARTIAL`, reused `/healthz`, confirmed the intended Red, preserved compliant Green leases, and used the owner-authorized one-file Yoga correction. DPL-DEC-041 remains explicit post-MVP declaration-compatibility debt. Commit `e68027b29d03144f6b69e57e6f04e5c7c99f1489` failed Codegen drift under Windows CRLF and is preserved. Reviewed follow-up `0659d1ee687c83aeb7fead9576ee60957544a78e` fixed generated LF materialization and passed exact install, pins, drift, focused 5/5, affected 6/6, typecheck, docs/ADR validation, root build, diff and marker audits, but its final frozen-test byte check exposed CRLF working materialization despite the exact committed blob and clean status. One exact LF test attribute now restores the recorded filesystem hash without changing test or product content, and focused correction review passes with no finding. Pre-existing Sequelize/uuid advisories remain separately scoped pre-production maintenance, not implicit post-MVP deferral. A preserved correction commit and complete new proof remain. No AC advance or full NFR-003 claim exists; Milestones 2 through 4 remain unstarted.
...
Committed HEAD still has the activation-era Express-only product API because the reviewed Milestone 1 candidate remains uncommitted. The candidate adds exact DPL-DEC-040 manifest/lock entries, an injected conditional `/graphql` seam, a checked-in query schema, generated resolver types, thin resolvers/handler, explicit application interfaces, and DPL-DEC-041's API-local declaration exception while preserving `createApp()` as health-only composition. It passes the full local Milestone 1 join and fresh S2 review; only documentation confirmation, the primary exact-tree commit, and clean-checkout proof remain before acceptance. `apps/api/src/server.ts` still parses the loopback host/port and starts the health-only composition; production PostgreSQL composition remains Milestone 2. `vitest.config.ts` already registers `api-unit`, `api-application`, and serial `api-persistence-integration`; root `test:integration` is active. `withPostgresNamespace` plus `prepareMigratedNamespace` provides run-owned real-PostgreSQL setup and cleanup.
...
- Tree state: one exact LF test attribute and living evidence are modified after the second failed proof; follow-up commit, clean-checkout proof, and milestone acceptance remain pending.
