...
`apps/api/src/app.ts` currently creates an Express app and owns only `GET /healthz`. `apps/api/src/server.ts` parses the loopback host/port and starts the process. `apps/api/src/config.ts` owns only API host/port. `apps/api/package.json` has Express, Sequelize, PostgreSQL, and migration dependencies but no GraphQL runtime. `vitest.config.ts` already registers `api-unit`, `api-application`, and serial `api-persistence-integration`; root `test:integration` is active. `withPostgresNamespace` plus `prepareMigratedNamespace` provides run-owned real-PostgreSQL setup and cleanup.
