...
- Decision: Enforce every TASK-003 worker lease with the coordinator-owned automatic write-lease guard before accepting Setup, Red, Green, Refactor, or evidence-document handoffs.
...
Plan registration, its hash-specific independent final evidence review, the later worker-topology revision, and the automatic-guard revision's complete fresh `PASS` are the only completed outcomes. The reviewed hashes remain point-in-time evidence. The automatic guard makes each planned worker assignment machine-verifiable within its documented endpoint-state boundary, and this plan is ready for a separate execution instruction. TASK-003 remains `Pending`; no dependency, command, application process, infrastructure container, product test, browser binary, or acceptance evidence has been created. The plan remains restartable, assigns serial write ownership, and surfaces the unresolved CI evidence boundary before code can be mistaken for complete delivery.
...
When execution is separately authorized, follow the [worker-first ExecPlan implementation workflow](../../.codex/execplan-implementation-workflow.md) and its [automatic write-lease guard](../../.codex/write-lease-guard.md). Exact leases are issued one turn at a time after Milestone 1 records the final paths and commands. Immediately before each spawn, the primary starts the guard with the exact workflow, task, cycle, lease, phase, attempt, owner, role, allowed files or directory roots, and forbidden files or directory roots, then retains its contract digest. After the handoff, the primary terminally closes that pinned lease. Only a fresh compliant closure permits the barrier in the table below; a replay additionally requires pinned status with no post-close drift. A violation or guard error freezes further writes for triage. The anticipated ownership below is controlling unless current repository evidence requires the primary to narrow it further:

| Slice | Writer and mode | Leased path family | Barrier before the next writer |
|---|---|---|---|
| Task start, reversible DPL choices, authority state, and ignore controls | Primary coordinator | This ExecPlan, root current status, plan index, canonical TASK-003 record, execution log, and the exceptional targeted `.gitignore` update before any worker baseline | TASK-003 is `In progress`, required execution decisions and ignored-output boundaries exist, the exceptional primary edit is recorded and validated, and no implementation file was added early. |
| Red-enabling workspace bootstrap | `code_worker` in `SETUP` mode | One ordered lease covering approved manifests, lockfile, root/package TypeScript and test configuration, `.env.example`, and package metadata; `.gitignore` is frozen and forbidden | Structural validation passes and no production behavior, ignore-control change, or inactive test scope exists. |
...
Only one agent owns a path at a time, and only one automatic worker lease may be active in this worktree. `test_worker` and `code_worker` never write concurrently within the same cycle. Each lease permits one initial turn and at most one coordinator-authorized correction. The worker echoes the lease ID and contract digest but never invokes the guard, changes its baseline, or edits its ignored runtime records. Once Red is accepted, a material test change ends that cycle and requires fresh Red; an exhausted worker correction or a needed path, dependency, semantic, authority expansion, guard violation, or ambiguous concurrent change returns to the primary. A correction uses a new lease and fresh baseline after the prior lease is terminal. Final integrated review retains the repository's two-cycle correction limit.
...
For each cycle, each worker returns the exact final command, nonzero Red diagnostic, Green pass summary, post-Refactor pass summary, changed paths, lease ID, contract digest, and correction count in its fixed handoff. The primary terminally closes the pinned lease, accepts the barrier only from a fresh compliant closure or a replayed compliant receipt followed by pinned status with no post-close drift, verifies the repository state, and then records or delegates a separately guarded evidence-only lease for `Progress` and `Artifacts and Notes`; only the primary updates authoritative status and the task's execution chronology. Do not preserve the placeholder once a command becomes authoritative.
...
Every Red must fail for the intended reason before its production edit. A dependency-resolution, unrelated type error, stale process, occupied port, or previous failing test is not acceptable Red evidence. `test_worker` returns the test diff and Red packet to the primary, which first obtains a compliant terminal lease receipt and then accepts or rejects that barrier. `code_worker` must reproduce the accepted Red, may add only enough production behavior for the current failure, and may not modify the frozen test. Refactor may not add behavior and must repeat the relevant scope. A missing or noncompliant receipt keeps the barrier unaccepted even when the focused command succeeds. Exact transcripts replace the generic expectations in the table as execution proceeds.
