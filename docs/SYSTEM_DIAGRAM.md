...
- Status: Target product architecture; TASK-003 foundation, TASK-004 migrations, TASK-005 import, TASK-006 GraphQL reads, and TASK-007 bounded Redis cache-aside realized; mutations and the browser product path remain incomplete
...
- [ADR-0016](./adrs/0016-use-milestone-slice-tdd-with-independent-test-and-implementation-ownership.md) and [DG-001](./IMPLEMENTATION_PLAN.md#dg-001---typescript-test-harness) govern current verification. TASK-003 implemented the unit, application, and Chromium process-smoke scopes; TASK-004 implemented the real-PostgreSQL integration scope under now-Superseded ADR-0010; TASK-007 added real-Redis cache and invalidation evidence through preflight-classified milestone slices. Its product architecture remains implemented, while hosted closure is reopened only for the missing run-isolated CI namespace. Later owners activate only their task-specific scopes. Test tooling is intentionally not represented as a runtime module.
