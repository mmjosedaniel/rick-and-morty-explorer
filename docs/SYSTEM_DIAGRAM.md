...
- Status: Target product architecture; TASK-003 foundation, TASK-004 migrations, TASK-005 import, TASK-006 GraphQL reads, TASK-007 bounded Redis cache-aside, TASK-008 backend mutations, and TASK-010 list browser path realized; detail UI, frontend interactions/refetching, and final responsive/resilient closure remain incomplete
...
flowchart LR
    ...
    subgraph WEB["apps/web — React 18 · React Router DOM · Tailwind CSS"]
        ...
        CLIENT["Frontend server-data boundary<br/>TanStack Query cache · Project-owned typed GraphQL executor<br/>Accepted ADR-0017 · Implementation pending"]
    ...
    subgraph API["apps/api — Express modular monolith"]
        ...
        PERSISTENCE["Sequelize persistence adapters<br/>Read, import, and interaction mutation repositories implemented"]
...
- PostgreSQL is the runtime source of truth and initializes on the first list, detail, or comment read or favorite/comment mutation. A mutation-first request does not construct Redis. Redis is a finite-lived list-search optimization, remains disconnected until list-cache demand, and falls back to PostgreSQL on a miss or failure.
...
- Now-Superseded [ADR-0012](./adrs/superseded/0012-use-a-build-first-programmatic-migration-lifecycle.md) historically resolved [DG-002](./IMPLEMENTATION_PLAN.md#dg-002---sequelize-migration-lifecycle). Accepted [ADR-0015](./adrs/0015-use-a-build-first-migration-lifecycle-with-exact-catalog-byte-lock-identity.md), prepared by completed [TASK-018](./IMPLEMENTATION_PLAN.md#task-018---resolve-the-postgresql-migration-lock-namespace-identity), is the current whole-record build-first migration authority and replaces ADR-0012's NFC identity with a restricted-ASCII catalog-bound v2 identity. Fresh independent review returned `PASS` on exact proposal SHA-256 `8B7B9EC9508DF01E57EA067344896814CD0B0B1B3D8083B889C7ED44AA5432B1`, and the project owner explicitly approved those bytes on 2026-08-14. [DG-005](./IMPLEMENTATION_PLAN.md#dg-005---postgresql-migration-lock-namespace-identity) is Resolved. Separately authorized TASK-004 is `Complete`; its task-scoped artifact, migration, Sequelize mapping, status/rollback, failure, lock, concurrency, command, verification-controller, PostgreSQL 18.6, clean-checkout, hosted-CI, fresh-review, and documentation-closure boundaries pass. Accepted [ADR-0017](./adrs/0017-use-tanstack-query-with-a-project-owned-typed-graphql-executor.md) resolves [DG-003](./IMPLEMENTATION_PLAN.md#dg-003---frontend-graphql-client-and-query-cache) with TanStack Query plus a project-owned typed GraphQL executor; this target boundary is not implementation evidence.
- Accepted [ADR-0014](./adrs/0014-persist-and-deliver-character-image-urls-directly.md) resolves [DG-006](./IMPLEMENTATION_PLAN.md#dg-006---character-image-url-successor-boundary): the importer persists only the exact validated absolute `Character.image` URL, GraphQL and finite Redis projections return that URL, and the browser requests the avatar directly. The application owns no image bytes, decoder, proxy, asset route, or image lifecycle. ADR-0001, ADR-0004, and ADR-0013 are Superseded history. [AUTH-001](./IMPLEMENTATION_PLAN.md#auth-001---character-image-content-rights-authorization) is `Authorized` only within the recorded non-commercial portfolio/direct-URL scope; the implementation plan owns scope-bound continuity and reopen conditions, while ADR-0014 owns the direct-URL technical boundary. TASK-005 exact URL ingestion, TASK-006 exact stored-URL GraphQL projection, TASK-007's finite namespaced Redis projection and post-import invalidation, and TASK-010/TASK-011 native list/detail avatar delivery are implemented; failure fallback remains pending TASK-012.
- TASK-008 implements favorite/comment writes through the global single-user demonstration semantics accepted by ADR-0005 and reads them back through TASK-006 detail queries. Completed TASK-011 supplies the detail UI, ID-only frontend favorite operation, validated comment interaction, and exact awaited detail refetch. Public anonymous writes or user accounts still require ADR-0005's security and ownership follow-up before deployment.
- [ADR-0016](./adrs/0016-use-milestone-slice-tdd-with-independent-test-and-implementation-ownership.md), accepted [ADR-0018](./adrs/0018-define-the-typescript-test-harness-with-repository-owned-smoke-orchestration.md), and [DG-001](./IMPLEMENTATION_PLAN.md#dg-001---typescript-test-harness) govern current verification. TASK-003 implemented the unit, application, and Chromium process-smoke scopes; TASK-004 implemented the real-PostgreSQL integration scope under now-Superseded ADR-0010; completed TASK-007 added real-Redis cache and invalidation evidence through preflight-classified milestone slices and passes its merged run-isolated hosted-CI boundary. ADR-0018 preserves those scopes and replaces only Playwright-owned smoke servers with the repository TypeScript orchestrator; TASK-010's Windows and disposable-Ubuntu seven-case evidence closes its changed-boundary proof. Later owners activate only their task-specific scopes. Test tooling is intentionally not represented as a runtime module.
...
- [ADR-0001: Modular monolith workspace (`Superseded`)](./adrs/superseded/0001-use-a-modular-monolith-workspace.md)
...
- [ADR-0004: Database as runtime source of truth (`Superseded`)](./adrs/superseded/0004-use-the-database-as-the-runtime-source-of-truth.md)
...
- [ADR-0016: Milestone-slice TDD with independent test and implementation ownership](./adrs/0016-use-milestone-slice-tdd-with-independent-test-and-implementation-ownership.md)
- [ADR-0011: Superseded TypeScript test harness](./adrs/superseded/0011-define-the-typescript-test-harness.md)
- [ADR-0018: TypeScript test harness with repository-owned smoke orchestration](./adrs/0018-define-the-typescript-test-harness-with-repository-owned-smoke-orchestration.md)
- [ADR-0012: Superseded build-first programmatic migration lifecycle](./adrs/superseded/0012-use-a-build-first-programmatic-migration-lifecycle.md)
...
- [ADR-0017: TanStack Query with a project-owned typed GraphQL executor (`Accepted`)](./adrs/0017-use-tanstack-query-with-a-project-owned-typed-graphql-executor.md)
