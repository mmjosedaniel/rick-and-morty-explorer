...
- Status: Target product architecture; TASK-003 foundation, TASK-004 migrations, TASK-005 import, TASK-006 GraphQL reads, TASK-007 bounded Redis cache-aside, and TASK-010 list browser path realized; mutations, detail UI, and final responsive/resilient closure remain incomplete
...
- PostgreSQL is the runtime source of truth and initializes on the first list, detail, or comment read. Redis is a finite-lived list-search optimization, remains disconnected until list-cache demand, and falls back to PostgreSQL on a miss or failure.
