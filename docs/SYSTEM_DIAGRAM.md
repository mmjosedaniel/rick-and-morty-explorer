...
flowchart LR
    ...
    subgraph SETUP["Initialization and ingestion"]
        MIGRATIONS["Version-controlled migrations<br/>Build-first programmatic Umzug<br/>Immutable emitted ESM (ADR-0012)<br/>Lock successor pending DG-005"]
...
- Accepted [ADR-0012](./adrs/0012-use-a-build-first-programmatic-migration-lifecycle.md) historically resolves [DG-002](./IMPLEMENTATION_PLAN.md#dg-002---sequelize-migration-lifecycle) and defines the target build-first programmatic migration boundary; no runner or migration exists yet. [DG-005](./IMPLEMENTATION_PLAN.md#dg-005---postgresql-migration-lock-namespace-identity) separately blocks TASK-004 until an owner-approved successor replaces ADR-0012's NFC-based migration-lock identity with a new literal, without reopening DG-002 or changing the task graph. [DG-003](./IMPLEMENTATION_PLAN.md#dg-003---frontend-graphql-client-and-query-cache) remains generic until an owner-approved ADR selects the frontend GraphQL client.
